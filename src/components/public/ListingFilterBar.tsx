...
export function ListingFilterBar({ islands, basePath, mapSlot, homeTypes = [], facilityTypes = [], bedroomOptions = [], bathroomOptions = [], showerOptions = [], showViewToggle, collapsibleFilters }: Props) {
    ...
    const hasActiveType = !!searchParams.get('type');
    const [filtersOpen, setFiltersOpen] = useState(
        collapsibleFilters ? (hasActiveType || hasActiveRoomFilters) : false
    );
    ...
    const filtersButton = hasRoomFeatures ? (
        <button
            onClick={() => setFiltersOpen(v => !v)}
            className={`flex items-center gap-2 text-sm font-semibold px-4 py-2 rounded-lg border-2 transition-colors flex-shrink-0 ${
    ...
    const searchRow = (
        <div className="flex flex-col gap-3">
            {/* Mobile: search + islands on one row */}
            <div className="flex lg:hidden gap-3 items-start">
                {searchInput}
                {islandsDropdown}
                {typeDropdown}
            </div>
            {/* Mobile: filters + view toggle centered */}
            {(hasRoomFeatures || showViewToggle) && (
                <div className="flex lg:hidden justify-center gap-3 items-center">
                    {filtersButton}
                    {viewToggleButtons}
                </div>
            )}
    ...
    const filterContent = collapsibleFilters ? (
        <div>
            {searchRow}
            {collapsibleContent}
        </div>
